...
my ($situation, $datadir ,$workdir, $method, $filetype, $seqtype, $identity, $evalue,$tpm, $norm, $randomsample, $thread, $outdir,$help);
GetOptions(
...
#software
my $blast = "$datadir/data/Softwares/ncbi-blast-2.13.0+/bin";
my $seqkit = "$datadir/data/Softwares/seqkit";
my $diamond = "$datadir/data/Softwares/diamond";
my $usearch = "$datadir/data/Softwares/usearch11";
my $csvtk = "$datadir/data/Softwares/csvtk";
...
my %id2gene;
my (@genes,%ha);
open(FILE, "$datadir/data/id2gene.CCycDB.map") || die "#ERROR: cannot open id2gene.map !\n";
...
my %annotation;
open(ANNO, "$datadir/data/annotations.txt") || die "#ERROR: cannot open annotation.txt ! \n";
while (<ANNO>) {
  chomp;
  my @items = split("\t", $_);
...
if ($situation ne "tabular" && $method eq "diamond") {
  ...
  my $diamond_db = "$diamond makedb --in $datadir/data/CCycDB.database --db $datadir/data/CCycDB.database" if (!-e "$datadir/data/CCycDB.database.dmnd");
  system("$diamond_db") if (!-e "$datadir/data/CCycDB.database.dmnd");
  ...
  foreach my $file (@files) {
	...
	print "Mapping:$file\n";
	if(!-e $out){
	print "$diamond blastx $diamond_parameters -d $datadir/data/CCycDB.database -q $file -o $out\n";
	system("$diamond blastx $diamond_parameters -d $datadir/data/CCycDB.database -q $file -o $out") if $seqtype eq "nucl";
	system("$diamond blastp $diamond_parameters -d $datadir/data/CCycDB.database -q $file -o $out") if $seqtype eq "prot";
	}
  }
  ...
  die "#ERROR: Please specify the location of usearch!" if !-e $usearch;
	...
	if(!-e $out){
	system("$usearch -usearch_global $file -db $datadir/data/CCycDB.database $usearch_parameters -blast6out $out");
	}
  }
  
}elsif ($situation ne "tabular" && $method eq "blast") {
  die "#ERROR: Please specify the location of blast and/or formatdb! \n" if !-e $blast;
  if ($filetype !~ /fasta|fa/) {
  #if ($filetype =~ /gz|fastq/) {
    die "#ERROR: Only fasta files(filetype is \"fasta|fa\") are supported by blast program!\n";
  }
  @files = glob("$workdir/*$filetype");
  system("$blast/makeblastdb -in $datadir/data/CCycDB.database -dbtype prot -out $datadir/data/CCycDB.blast");
  
  foreach my $file (@files) {
    my $out = $file;
    $out =~ s/$filetype/blast/;
	if(!-e $out){
    system("$blast/blastp -db $datadir/data/CCycDB.blast -query $file -out $out $blast_parameters") if $seqtype eq "prot";
    system("$blast/blastx -db $datadir/data/CCycDB.blast -query $file -out $out $blast_parameters") if $seqtype eq "nucl";
	}
  }
}
print "done!\n";
...
if ($norm == 0){
 ...
 print FOUT "Gene\tMean Identity\tAnnotation\tKO\tEC_number\tCAZY\tCAZY_group\teggNOG\t",join("\t", @samplelist), "\n";
 foreach my $gkey (sort keys %identity){
  print FOUT "$gkey";
  if ($identity{$gkey}){
	  #my $identity = sprintf("%.2f",sum(@{$identity{$gkey}}) / scalar(@{$identity{$gkey}}));
	  my $avg_id_val = sum(@{$identity{$gkey}}) / scalar(@{$identity{$gkey}});
	  my $avg_id_fmt = sprintf("%.2f", $avg_id_val);
 ...
 open(FOUT, ">$outdir\/FunProfile_$situation\_$method\_randomTO$rs.txt") || die "#ERROR: cannot write outfile\n";
 print FOUT "#Random sampling: $rs\n";
 print FOUT "Gene\tMean Identity\tAnnotation\tKO\tEC_number\tCAZY\tCAZY_group\teggNOG\t",join("\t", @samplelist), "\n";
 foreach my $gkey (sort keys %abundance_rs) {
  print FOUT "$gkey\t";
  if ($identity{$gkey}){
	my $avg_raw = sum(@{$identity{$gkey}}) / scalar(@{$identity{$gkey}});
	my $avg_fmt = sprintf("%.2f", $avg_raw);
	print FOUT "\t$avg_fmt";
  }else{
	print FOUT "\t0";
  }
...
my %pathway;
open(SUBIII, "$datadir/data/subcategoryII.txt") || die "# cannot open subcategoryIII.txt";
while (<SUBIII>) {
  chomp;
  my @items = split("\t", $_);
...
sub PrintHelp() {
  print "Manual:\n";
  print
    "perl CCycdb.pl [-situation situation] [-datadir datafile_path][-wd work_directory] [-m diamond|usearch|blast] [-f filetype] [-s seqtype] [-tpm] [-norm] [-rs random_sampling_size] [-thread] [-od out_directory] [-h help]\n\n";
  print "E.g. perl CCycdb.pl -situation read-based -datadir /clusters -wd ./ -m diamond -f fasta -s nucl -norm 0 -thread 20 \n\n";
  
  print "E.g. perl CCycdb.pl -situation assembly-based -datadir /clusters -wd ./ -m diamond -f fasta -s nucl -tpm 1 -thread 60 \n\n";
  
  print "-situation\tthe situation for input files(read-based|assembly-based|tabular)\n";
  print "-datadir\t<data> directory from github should be included in this directory\n";
  print "-wd\twork directory include your fasta/fastq files\n";
  print "-od\toutfile directory(E.g. ./result)\n";
